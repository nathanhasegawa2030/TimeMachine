###nonuniformCorrelatedMinor.R

#Written by Nathan Hasegawa

#Generates synthetic data where the predictors and responses are not uniformly
#distributed, and there are more predictors than responses. Overfitting is a
#serious concern.
...
folderpath <- "Desktop/NSH_WI26_Rotation/nonuniformCorrelatedMinorImages"

#Import the training data
train <- read.csv("Desktop/NSH_WI26_Rotation/SyntheticData/nonuniformCorrelatedMinorTrain.csv")
y <- train[,1]
x <- train[,-1]
...
#Import test data
test <- read.csv("Desktop/NSH_WI26_Rotation/SyntheticData/nonuniformCorrelatedMinorTest.csv")
y.test <- test[,1]
x.test <- test[,-1]
...
save(model.standard, model.mgaussian, test, train, alpha, resid.test.std,
     ...
     file="Desktop/NSH_WI26_Rotation/Workspaces/nonuniformCorrelatedMinor.RData")

#CAN START HERE

load("Desktop/NSH_WI26_Rotation/Workspaces/nonuniformCorrelatedMinor.RData")
folderpath <- "Desktop/NSH_WI26_Rotation/nonuniformCorrelatedMinorImages"
...
#correct amount is 5)
sin.par <- sin.top[sin.top$nonzero < 15,]
cos.par <- cos.top[cos.top$nonzero < 15,]
mga.par <- mga.top[mga.top$nonzero < 15,]
