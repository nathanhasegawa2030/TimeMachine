...
folderpath <- "Desktop/NSH_WI26_Rotation/standardOverdeterminedMajorImages"

#Import the training data
train <- read.csv("Desktop/NSH_WI26_Rotation/SyntheticData/standardOverdeterminedMajorTrain.csv")
y <- train[,1]
x <- train[,-1]
...
save(model.standard, model.mgaussian, test, train, alpha, resid.test.std,
     ...
     file="Desktop/NSH_WI26_Rotation/Workspaces/standardOverdeterminedMajor.RData")

#CAN START HERE

load("Desktop/NSH_WI26_Rotation/Workspaces/standardOverdeterminedMajor.RData")
...
sin.top <- sin.agg[sin.agg$r2 > 0.73,]
cos.top <- cos.agg[cos.agg$r2 > 0.73,]
mga.top <- mga.agg[mga.agg$r2 > 0.73,]
